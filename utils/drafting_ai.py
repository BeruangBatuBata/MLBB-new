...
def train_and_save_prediction_model(matches, HERO_PROFILES, HERO_DAMAGE_TYPE, model_filename='draft_predictor.joblib'):
    ...
    for match in matches:
        ...
        for game in match.get('match2games', []):
            ...
            def get_tags_for_team(team_picks, HERO_PROFILES):
                team_tags = defaultdict(int)
                has_frontline = any('Front-line' in p['tags'] for h in team_picks if h in HERO_PROFILES for p in HERO_PROFILES[h])
                for hero in team_picks:
                    profiles = HERO_PROFILES.get(hero)
                    if profiles:
                        build = next((p for p in profiles if 'Tank' in p['build_name']), profiles[0]) if len(profiles) > 1 and not has_frontline else profiles[0]
                        for tag in build['tags']: team_tags[tag] += 1
                return team_tags
            
            blue_tags, red_tags = get_tags_for_team(blue_picks_list, HERO_PROFILES), get_tags_for_team(red_picks_list, HERO_PROFILES)
            ...
            vector[feature_to_idx[match_teams[0]]] = 1; vector[feature_to_idx[match_teams[1]]] = -1
            X.append(vector); y.append(1 if game['winner'] == '1' else 0)
    
    if not X: raise ValueError("Could not generate training samples.")
...
def predict_draft_outcome(blue_picks, red_picks, blue_team, red_team, model_assets, HERO_PROFILES):
    ...
    for role, hero in red_picks.items():
        if hero in all_heroes and f"{hero}_{role}" in feature_to_idx: vector[feature_to_idx[f"{hero}_{role}"]] = -1
    def get_tags_for_team(team_picks, HERO_PROFILES):
        team_tags = defaultdict(int); team_list = list(team_picks.values())
        has_frontline = any('Front-line' in p['tags'] for h in team_list if h in HERO_PROFILES for p in HERO_PROFILES[h])
        for hero in team_list:
            ...
            if profiles:
                ...
                for tag in build['tags']: team_tags[tag] += 1
        return team_tags
    blue_tags, red_tags = get_tags_for_team(blue_picks, HERO_PROFILES), get_tags_for_team(red_picks, HERO_PROFILES)
    for tag, count in blue_tags.items():
        if f"blue_{tag}_count" in feature_to_idx: vector[feature_to_idx[f"blue_{tag}_count"]] = count
    for tag, count in red_tags.items():
        if f"red_{tag}_count" in feature_to_idx: vector[feature_to_idx[f"red_{tag}_count"]] = count
